...
{
	const char *key = "gDnToNSv65iaoPPb";
	unsigned char iv[17] = "gDnToNSv65iaoPPb";
	const unsigned char before[64] = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ\n";
	unsigned char after[64]={0};
	int before_len = strlen(before);

	printf("AES CBC 128 Enc Test:\n");
	printf("Key: %s\n", key);
	printf("Key-len: %d\n", strlen(key));
	printf("before: %s\n", before);
	printf("Data-len: %d\n", before_len);

	AES128_CBC_encrypt_buffer(after, before, strlen(before), key, iv);
